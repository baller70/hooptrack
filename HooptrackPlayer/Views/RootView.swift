...
@MainActor
final class PlayerWebSession: ObservableObject {
    ...
    var screenshotIdentifier: String {
        ...
        #endif
        return "hooptrack-player-web-app"
    }
    ...
    func open(_ deepLink: URL) {
        let path = deepLink.path.isEmpty ? "/player" : deepLink.path
    ...
    func navigationFinished(in webView: WKWebView) {
        if webView.url?.path == "/login", attemptFactoryLogin(in: webView) {
            return
        }
        isLoading = false
        errorMessage = nil
    ...
    private func attemptFactoryLogin(in webView: WKWebView) -> Bool {
        ...
        let route = javascriptJSON(["path": screenshotScene?.routePath ?? "/player", "nonce": UUID().uuidString])
        let script = """
...
private struct PlayerWebView: UIViewRepresentable {
    @ObservedObject var session: PlayerWebSession
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate {
        private let session: PlayerWebSession
        private weak var webView: WKWebView?

        init(session: PlayerWebSession) {
